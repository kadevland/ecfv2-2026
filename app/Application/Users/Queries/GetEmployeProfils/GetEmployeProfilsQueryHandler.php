...
namespace App\Application\Users\Queries\GetEmployeProfils;
...
final class GetEmployeProfilsQueryHandler implements QueryHandlerInterface
{
    public function handle(QueryInterface $query): Result
    {
        if (!$query instanceof GetEmployeProfilsQuery) {
            return Result::error(
                'INVALID_QUERY_TYPE',
        ...
        try {
            ...
            $queryBuilder = UserProfil::query()
                ->where(UserProfilSchema::TYPE, 'employee');
            ...
            return Result::error(
                'QUERY_FAILED',
                'Erreur lors de la récupération des profils employés: ' . $e->getMessage()
            );
        }
    }

    /** @param \Illuminate\Database\Eloquent\Builder<UserProfil> $queryBuilder */
    private function applyFilters(\Illuminate\Database\Eloquent\Builder $queryBuilder, GetEmployeProfilsQuery $query): void
    {

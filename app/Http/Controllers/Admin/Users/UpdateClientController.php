...
use App\Http\Mappers\Admin\Users\UpdateClientRequestMapper;

final class UpdateClientController extends Controller
{
    public function __construct(
        private readonly CommandBus $commandBus
    ) {}

    public function __invoke(UpdateClientRequest $request, string $uuid): RedirectResponse
    {
        try {
            // 1. Mapper Request → Command
            $command = UpdateClientRequestMapper::toCommand($request, $uuid);
            ...
            return redirect()
                ->route('admin.users.clients.show', $uuid)

...
@section('title', 'Modifier Salle - ' . $salle->nom)
...
@section('breadcrumbs')
    <nav class="flex space-x-2 text-sm text-gray-600">
        ...
        <span>/</span>
        <a href="{{ route('admin.salles.show', $salle->uuid) }}" class="hover:text-gray-900">{{ $salle->nom }}</a>
        <span>/</span>
...
@section('actions')
    <div class="flex space-x-3">
        <x-button href="{{ route('admin.salles.show', $salle->uuid) }}" variant="outlined" theme="admin" icon="heroicon-o-eye">
            Voir
        </x-button>
...
@section('content')
    {{-- Form principale --}}
    <form method="POST" action="{{ route('admin.salles.update', $salle->uuid) }}" id="salle-form" class="sr-only">
        @csrf
        @method('PUT')
    </form>
    ...
    <div class="flex flex-col gap-6">
        ...
        <x-card theme="admin" title="Informations générales" subtitle="Nom et cinéma de la salle" size="md" variant="shadow" class="bg-white">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    <label for="cinema_display" class="block text-sm font-medium text-gray-700 mb-2">
                        Cinéma
                    </label>
                    {{-- Champ hidden pour préserver la valeur --}}
                    <input type="hidden" name="cinema_uuid" value="{{ $salle->cinemaUuid }}" form="salle-form">
                    {{-- Affichage read-only du cinéma --}}
                    <div id="cinema_display"
                        class="w-full px-3 py-2 bg-gray-50 border border-gray-200 rounded-md text-gray-700">
                        {{ $salle->cinemaNom }} - {{ $salle->cinemaVille }}
                    </div>
                    <p class="mt-1 text-xs text-gray-500">
                        <i class="heroicon-o-information-circle inline w-3 h-3"></i>
                        Le cinéma ne peut pas être modifié après création
                    </p>
                </div>
                ...
                <div>
                    ...
                    <input type="text" id="nom" name="nom" form="salle-form" value="{{ old('nom', $salle->nom) }}" required
                ...
                <div>
                    ...
                    <select id="statut" name="statut" form="salle-form" required
                        ...
                        <option value="ACTIVE" {{ old('statut', $salle->statut) === 'ACTIVE' ? 'selected' : '' }}>Active</option>
                        <option value="MAINTENANCE" {{ old('statut', $salle->statut) === 'MAINTENANCE' ? 'selected' : '' }}>En maintenance</option>
                        <option value="RENOVATION" {{ old('statut', $salle->statut) === 'RENOVATION' ? 'selected' : '' }}>En rénovation</option>
                        <option value="HORS_SERVICE" {{ old('statut', $salle->statut) === 'HORS_SERVICE' ? 'selected' : '' }}>Hors service</option>
                    </select>
                    @error('statut')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
        ...
        <x-card theme="admin" title="Configuration des places" subtitle="Capacité et disposition de la salle" size="md" variant="shadow" class="bg-white">
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div>
                    ...
                    <input type="number" id="capacite_totale" name="capacite_totale" form="salle-form"
                        value="{{ old('capacite_totale', $salle->capaciteTotale) }}" required min="10" max="1000"
                ...
                <div>
                    ...
                    <input type="number" id="nombre_rangees" name="nombre_rangees" form="salle-form"
                        value="{{ old('nombre_rangees', $salle->nombreRangees ?? '') }}" required min="1" max="50"
                ...
                <div>
                    ...
                    <input type="number" id="places_par_rangee" name="places_par_rangee" form="salle-form"
                        value="{{ old('places_par_rangee', $salle->placesParRangee ?? '') }}" required min="1" max="50"
                ...
                <div>
                    ...
                    <input type="number" id="places_standard" name="places_standard" form="salle-form"
                        value="{{ old('places_standard', $salle->placesStandard ?? 0) }}" min="0"
                ...
                <div>
                    ...
                    <input type="number" id="places_pmr" name="places_pmr" form="salle-form"
                        value="{{ old('places_pmr', $salle->placesPmr ?? 0) }}" min="0"
        ...
        <x-card theme="admin" title="Équipements techniques" subtitle="Configuration audio-visuelle" size="md" variant="shadow" class="bg-white">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                        ...
                        @foreach($qualitesProjection as $qualite)
                            <label class="flex items-center">
                                <input type="checkbox" name="qualite_projection[]" value="{{ $qualite->value }}" form="salle-form"
                                    {{ in_array($qualite->value, old('qualite_projection', $salle->qualiteProjection ?? [])) ? 'checked' : '' }}
                            ...
                            @if($qualite->value !== 'STEREO')
                                <label class="flex items-center">
                                    <input type="checkbox" name="qualite_sonore[]" value="{{ $qualite->value }}" form="salle-form"
                                        {{ in_array($qualite->value, old('qualite_sonore', $salle->qualiteSonore ?? [])) ? 'checked' : '' }}
                                        class="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 rounded focus:ring-blue-500">
                                    <span class="ml-2 text-sm">{{ $qualite->getLabel() }}</span>
                                </label>
                            @endif
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mt-6">
                <div class="flex items-center">
                    ...
                    <input type="checkbox" id="climatisation" name="climatisation" value="1" form="salle-form"
                        {{ old('climatisation', $salle->climatisation) ? 'checked' : '' }}
                        class="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 rounded focus:ring-blue-500">
                ...
                <div class="flex items-center">
                    ...
                    <input type="checkbox" id="accessibilite_pmr" name="accessibilite_pmr" value="1" form="salle-form"
                        {{ old('accessibilite_pmr', $salle->accessibilitePmr) ? 'checked' : '' }}
                        class="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 rounded focus:ring-blue-500">
        ...
        <div class="flex justify-end space-x-4">
            <x-button href="{{ route('admin.salles.show', $salle->uuid) }}" variant="outlined" theme="admin">
                Annuler
            </x-button>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            function validateCapacity() {
                ...
                if (rangees > 0 && parRangee > 0 && capaciteCalculee !== capacite) {
                    showError(capaciteTotale, `Incohérence: ${rangees} × ${parRangee} = ${capaciteCalculee} ≠ ${capacite}`);
                    hasError = true;
                }
            ...
            document.getElementById('salle-form').addEventListener('submit', function(e) {
                if (!validateCapacity()) {
                    e.preventDefault();
                    alert('Veuillez corriger les incohérences dans la configuration des places avant de continuer.');
                }
            });
        });
    </script>

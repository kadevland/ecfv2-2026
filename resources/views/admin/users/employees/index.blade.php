...
@section('breadcrumbs')
    <nav class="flex space-x-2 text-sm text-gray-600">
        ...
        <span>/</span> --}}
        <span class="text-gray-900 font-medium">Employés</span>
    </nav>
@endsection

@section('subtitle', 'Gérez les comptes employés et administrateurs')
            ...
            <table class="min-w-full divide-y divide-gray-200">
                ...
                <tbody class="divide-y divide-gray-200">
                    ...
                    <tr class="hover:bg-gray-50">
                        <td class="px-6 py-4 whitespace-nowrap">
                            <div class="flex items-center">
                                <div class="flex-shrink-0 h-10 w-10">
                                    <div class="h-10 w-10 rounded-full {{ $employee->type === 'admin' ? 'bg-purple-100' : 'bg-green-100' }} flex items-center justify-center">
                                        <span class="{{ $employee->type === 'admin' ? 'text-purple-600' : 'text-green-600' }} font-medium text-sm">
                                            {{ strtoupper(substr($employee->prenom ?? 'U', 0, 1)) }}
                                        </span>
                                    </div>
                                </div>
                                <div class="ml-4">
                        ...
                        <td class="px-6 py-4 whitespace-nowrap">
                            <div class="text-sm text-gray-900">{{ $employee->email ?? 'Non renseigné' }}</div>
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                            <div class="relative" x-data="{ open: false }">
                                ...
                                <div x-show="open" @click.away="open = false" x-transition class="absolute right-0 z-10 mt-2 w-56 origin-top-right rounded-md bg-white shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none">
                                    <div class="py-1">
                                        <a href="{{ route('admin.users.employees.show', $employee->user_uuid) }}" class="flex px-4 py-2 text-sm text-gray-700 hover:bg-gray-100">
                                        ...
                                        <a href="{{ route('admin.users.employees.edit', $employee->user_uuid) }}" class="flex px-4 py-2 text-sm text-gray-700 hover:bg-gray-100">
                                            <svg class="mr-3 h-5 w-5 text-gray-400" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
                                                <path stroke-linecap="round" stroke-linejoin="round" d="M16.862 4.487l1.687-1.688a1.875 1.875 0 112.652 2.652L10.582 16.07a4.5 4.5 0 01-1.897 1.13L6 18l.8-2.685a4.5 4.5 0 011.13-1.897l8.932-8.931zm0 0L19.5 7.125M18 14v4.75A2.25 2.25 0 0115.75 21H5.25A2.25 2.25 0 013 18.75V8.25A2.25 2.25 0 015.25 6H10" />
                                            </svg>
                                            Modifier profil
                                        </a>
                                        <a href="{{ route('admin.users.employees.emploi.edit', $employee->user_uuid) }}" class="flex px-4 py-2 text-sm text-gray-700 hover:bg-gray-100">
                                            <svg class="mr-3 h-5 w-5 text-gray-400" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
                                        ...
                                        @if($employee->user?->is_active ?? true)
                                        <button type="button" class="flex w-full px-4 py-2 text-sm text-orange-700 hover:bg-orange-50" onclick="if(confirm('Êtes-vous sûr de vouloir désactiver cet employé ?')) { /* TODO: Deactivate action */ }">
                                        ...
                                        @else
                                        <button type="button" class="flex w-full px-4 py-2 text-sm text-green-700 hover:bg-green-50" onclick="if(confirm('Êtes-vous sûr de vouloir réactiver cet employé ?')) { /* TODO: Activate action */ }">
